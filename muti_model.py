...
class EncoderModel(tf.keras.Model):
    def __init__(self, num_layers=6, d_model=256, dff=512, num_heads=8, vocab_size=18, a=2, dropout_rate=0.1,
                 dense_dropout=0.1):
...
class CostomModel(tf.keras.Model):
    def __init__(self, encoder_share, dense_dropout=0.1,a=1):
        super(CostomModel, self).__init__()
        self.encoder = encoder_share
        self.fc1 = tf.keras.layers.Dense(256, activation=tf.keras.layers.LeakyReLU(0.1))
        self.dropout = tf.keras.layers.Dropout(dense_dropout)
        self.fc2 = tf.keras.layers.Dense(a,activation=tf.keras.activations.sigmoid)
