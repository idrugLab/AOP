...
str2num = {'<pad>':0 ,'H': 1, 'C': 2, 'N': 3, 'O': 4, 'S': 5, 'F': 6, 'Cl': 7, 'Br': 8, 'I': 9,
         'P': 10,'B': 11,'Si':12,'Na':13,'Se':14,'<unk>':15,'<mask>':16,'<global>':17}
...
class Graph_Classification_Dataset(object):  # 图分类任务数据集处理
    ...
    def get_data(self):
        ...
        featurizer_dc = dc.feat.RawFeaturizer()
        loader = dc.data.CSVLoader(tasks=['Label'], featurizer=featurizer_dc, feature_field='smiles')
        dataset_dc = loader.create_dataset(self.path)
        splitter = dc.splits.RandomStratifiedSplitter()
        trn_id, val_id, test_id = splitter.split(dataset_dc, seed=self.seed)
        train_data = data[data.index.isin(trn_id)]
        val_data = data[data.index.isin(val_id)]
        test_data = data[data.index.isin(test_id)]
        ...
        self.dataset1 = tf.data.Dataset.from_tensor_slices((df_train_data[self.smiles_field], df_train_data[self.label_field]))
        self.dataset1 = self.dataset1.map(self.tf_numerical_smiles).cache().padded_batch(batch_size=self.batch_size, padded_shapes=(
        tf.TensorShape([None]), tf.TensorShape([None, None]), tf.TensorShape([self.a]))).shuffle(1000).prefetch(self.batch_size)
        ...
        self.dataset2 = self.dataset2.map(self.tf_numerical_smiles).padded_batch(512, padded_shapes=(
            tf.TensorShape([None]), tf.TensorShape([None, None]), tf.TensorShape([self.a]))).cache().prefetch(tf.data.experimental.AUTOTUNE)
...
class Graph_Regression_Dataset(object):  #  图回归任务数据集处理
    ...
    def get_data(self):
        ...
        data = self.df
        train_ids, val_ids, test_ids = random_scaffold_split(data, sizes=(0.8, 0.1, 0.1), balanced= False,seed=self.seed)
        ...
        self.dataset2 = tf.data.Dataset.from_tensor_slices((df_test_data[self.smiles_field], df_test_data[self.label_field]))
        self.dataset2 = self.dataset2.map(self.tf_numerical_smiles).padded_batch(512, padded_shapes=(
            tf.TensorShape([None]), tf.TensorShape([None, None]), tf.TensorShape([self.a]))).cache().prefetch(512)
...
class Inference_Dataset(object):
    def __init__(self,sml_list,max_len=500,addH=True,padded_batch=64):
        self.vocab = str2num
        self.devocab = num2str
    ...
    def get_data(self):
        ...
        self.dataset = self.dataset.map(self.tf_numerical_smiles).padded_batch(self.padded_batch, padded_shapes=(
